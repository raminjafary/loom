...
const EnvSchema = z.object({
  ...
  //
  // Was 3, on the stated grounds that it matches the own experiment — three workers on one
  // repo. That reasoning still holds and the number no longer follows from it, because the
  // corporation put *planner* runs above those workers and they are active at the same
  // time: a root plus two areas already fills a limit of 3 before a single worker starts,
  // so a corporation was silently truncated to whichever subtree got there first. Observed
  // live — one sub-planner finished having started nothing.
  //
  // 6 is three workers plus the planners above them, which is the same experiment with the
  // tree the corporation actually produces. Still deliberately small: concurrency
  // multiplies both spend and the human attention the riskiest assumption is about, so it
  // should be raised knowingly rather than defaulted high. Raise it with
  // `MAX_DELEGATION_DEPTH`, never separately — depth without width is what starves a
  // subtree.
  MAX_CONCURRENT_RUNS_PER_WORKSPACE: z.coerce.number().int().positive().default(6),
  // How many delegation hops may separate a run from the root of its tree. 1 is the flat
  // fan-out Phase 2 shipped: a Planner and its workers, nothing below them. The default of
  // 2 admits exactly one layer of sub-planners — a root that delegates areas, sub-planners
  // that decompose them, workers that do the units — and stops there. Depth is the axis
  // that multiplies cost fastest, because each hop is a frontier-model run whose only
  // output is more runs; raising it should be a decision, like the concurrency limit beside
  // it.
  MAX_DELEGATION_DEPTH: z.coerce.number().int().positive().default(2),
  // How long an entry may sit `merging` before the queue gives up on it. This is the merge
  // queue's equivalent of the dead-run reaper, and it exists for a specific failure: a
  // server that dies mid-merge leaves a `merging` row that the unique partial index makes
  // unclaimable, so that repository's queue would stall permanently with nothing to notice
  // it.
  //
  // Longer than the gateway's own merge timeout, so a Runner that is merely slow is failed
  // by the request that is actually watching it rather than by a sweep that cannot tell
  // slow from dead. Where the raw transcript tier's blobs live. A directory rather than a
  // database because these are append-once chunks nobody queries — putting them in Postgres
