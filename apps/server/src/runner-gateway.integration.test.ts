...
beforeEach(async () => {
   ...
   * testing the DAG, the roster, the ledger and the board, and a gate in front of them
   * would make every one of them assert the gate instead. The tests that are about the gate
   * turn it back on for themselves, which keeps the default honest: nothing here silently
   * proves that ungated is normal.
   */
...
describe('runner-gateway: a mastery run reaches the Runner as one', () => {
  ...
  it('records a proposed cross-project relation, and refuses an invented one', async () => {
    ...
    // A concept nobody recorded is a relation to nothing, and is refused rather than
    // stored.
  ...
  it('hands a later ordinary run the map, which is what the whole artifact is for', async () => {
    ...
    // which is exactly the signal domain expertise says should rank it below one that
    // merged.
...
describe('runner-gateway: warm handoff', () => {
  ...
  /**
   * The nudge (mastery — "the threshold nudges; the agent asks; the cap refuses"), driven
   * by the frame that actually carries the measurement. This is the half that was missing:
  ...
  /**
   * Mastery: "the threshold is a setting with a sane default." The plumbing was shaped for
   * it — `limits?: { handoffThreshold?, handoffCapPerTree? }` — and every caller passed
   * `{}`, which is the smallest possible gap between what code is built for and what it
   * does.
   */
...
/**
 * The retention hook: a human is notified when a run needs them, rather than having to
 * go and look. Every case here is one where a human who is not watching would
...
/**
 * The foundation for concurrency: a workspace may run several agents at once, and
...
irrelevant for this test`
  ...
  it('runs several agents at once, and refuses past the workspace limit', async () => {
    ...
    // Read from config rather than hardcoded: this number moved once already, when The
    // corporation put planner runs above the workers the riskiest assumption sized it for,
    // and a test that hardcodes it fails for the wrong reason when it moves again.
...
describe('runner-gateway: the verification harness', () => {
  ...
  /**
   * An agent editing the checks that judge it is the failure the roadmap sequences the
   * tiers 3 and 4 behind, arriving from the other direction.
   */
...
describe('runner-gateway: raw transcript tier', () => {
  ...
  /**
   * The event-tiering design calls this tier "policy-bound". Discarding a branch is a human
   * saying they do not want the work kept, and the verbatim record of it is part of that.
   */
...
Decompose and delegate.`
   ...
   *
   * Only this level can prove it: the name a model writes has to resolve against a *team*,
   * the child has to start in the resolved repository rather than its planner's, and a name
   * the team did not declare has to be refused. All three are facts about the real dispatch
   * path.
   */
...
Decompose and delegate.`
  ...
  /**
   * The payoff. A worker that starts later is handed what earlier runs recorded — which is
   * the whole reason the worker-notes design exists, since clone-per-run means every run
  ...
  /**
   * The worker-notes design: path ownership "lets the platform warn about overlap *before*
   * tokens are spent". Before, not during — so the warning must exist by the time the first
  ...
  /**
   * The kanban, which the worker-notes design insists is the same object as the ledger: a
   * card *is* a run, so there is no second source of truth for what a swarm is doing.
   */
 ...
 *
 * These drive the whole road a delta travels: a human's message, a Planner re-entered with
 * the four inputs mid-flight steering names, a delta on the wire, and the cancellations and
 * child runs it turns into. What is asserted hardest is the *boundary* — a delta may only
...
Decompose and delegate.`
  ...
  it('re-enters the planner with the goal, the plan, the tree state and the message', async () => {
    ...
    // The four inputs a re-planning brief carries.
...
Decompose and delegate.`
  ...
  it('skips a dependent when its predecessor fails, rather than running it on a broken base', async () => {
    // The collaboration topology states this as a requirement, and it is the whole reason a
    // pipeline is riskier than a fan-out: "everything downstream inherits the mistake".
  ...
  it('refuses a cyclic plan whole, and names the loop', async () => {
    ...
    // Refused *whole*, unlike a path overlap, which warns and runs. The collaboration
    // topology draws that distinction itself: a cycle is not a guess about the future.
    expect(await client.agentRun.listChildren({ agentRunId: run.id })).toEqual([])
  ...
  it('shows the per-stage spend ceiling before any child starts', async () => {
    // The collaboration topology requires this by name: "`dependsOn` ships with per-stage
    // budget accounting visible before the plan is approved."
...
Decompose and delegate.`
  ...
  it('gives the reviewer no path ownership', async () => {
    // The collaboration topology: "no path ownership of its own". The ledger is where a
    // sibling reads a claim from, so a reviewer claiming its target's paths would tell
    // every other worker that the reviewer owns them.
 ...
 * validated. What only this level can show is the thing the fleet design is actually
 * worried about — "a fleet size that the runtime never reads is a number a human tunes and
 * a swarm ignores": a plan asking for more than the width has to be *warned about* before
 ...
 *
 * The rule for this canvas is the roadmap's: it "may only draw what the runtime executes,
 * so each of these has to be a field the platform already reads — never a decoration." So
 * the two tests that matter are the two readers: a Planner is *told* the expectation at
 * plan time, and a plan that ignores it is *warned about* before the first child starts.
 */
...
The prompt it started with.`
  ...
  /**
   * The self-improvement loop — the measurement that decides whether a self-edit was an
   * improvement.
   *
   ...
   * generator's context agrees with it", and agreement produced that way is
  ...
  it('starts a blinded verifier in its own session, and records a letter as a verdict', async () => {
    ...
    // Every option's text, and none of the three things the self-improvement loop
    // withholds.
    ...
    /**
     * And a verdict from a run that is not the verifier is refused. The frame carries no
     * set id precisely so this cannot be aimed: which search a session belongs to is what
     * the platform recorded when it started it.
     */
...
/**
 * The held-out screen. A replay item is `(repository @ commit, task,
