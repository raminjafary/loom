...
describe('runner-gateway: agent run event ingest', () => {
  ...
  it('runs the full approval round-trip: request → listPending → decide → relayed back to the Runner', async () => {
    ...
    // The card must show the exact argv, never a model-authored summary (PLAN.md §6 A3).
  ...
  /**
   * Idempotency (PLAN.md §6 "idempotency keys on run steps"). A Runner that
  ...
  /**
   * Approval SLA (PLAN.md §6). Driven by calling the sweep with a zero SLA
  ...
  /**
   * The kill switch, end to end over the real protocol (PLAN.md §6): the Runner
  ...
  /**
   * Run resumption reconciliation (PLAN.md §7 Phase 1). Both branches matter: a Runner
...
/**
 * PLAN.md §7 Phase 2's foundation: a workspace may run several agents at once, and
 ...
 * Planner (§3), which does not exist yet, and a human starting a "child" by hand
