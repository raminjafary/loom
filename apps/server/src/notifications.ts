...
/**
 * The `NotificationPort`'s web push adapter — what notifies a human when a run needs
 * them.
 *
 * Web push is end-to-end encrypted per RFC 8291: the push service routes an opaque
 * ciphertext it cannot read, and only the subscribing browser holds the key. That is why
 * this is the Phase 1 transport rather than email — it reaches a closed laptop without
 * handing a third party the contents. The payload still carries no tool arguments (see
 * buildNotification), because deciding from a notification is the failure mode effect-based
 * classification exists to prevent, encryption or not.
 */
