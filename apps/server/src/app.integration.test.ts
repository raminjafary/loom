...
describe('contract over HTTP', () => {
  ...
  /**
   * The kill switch (PLAN.md §6). `truncateDomainTables` deliberately spares
  ...
  it('rejects starting a run while the workspace is paused', async () => {
    ...
    try {
      ...
      // never need to exist — that ordering is the assertion (PLAN.md §6: a
      // paused workspace must leave no half-created run behind).
      await expect(
...
describe('contract completeness', () => {
  it('exposes every declared procedure on the client', () => {
    // Guards PLAN.md §4c: the contract is the only surface, so a procedure
...
/**
 * Notification targets over the real wire (PLAN.md §3/§4a). This app is built
