 ...
 *
 * Built from the **board's own payload**, not a second endpoint. The hierarchy is
 * already in the data — `parentRunId` and `relation` are columns on `agent_run` — and
 * a separate tree query would be a second source of truth for what a swarm's shape is,
 * which is the thing the worker-notes design refuses. The board and this panel disagreeing about a
 * run would be worse than not having the tree.
 *
...
/**
 * The shape lives in client-core, not here: the contract-first rule means a TUI must be able to render the
 * same tree without reimplementing how a parent, a subtotal or a root is decided.
 */
