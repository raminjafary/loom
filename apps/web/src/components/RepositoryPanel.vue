...
const emit = defineEmits<{
  ...
  /**
   * This repository's definition of done. Whole list, never one check: the order is a
   * dependency order, and a control that could only append would make "run the build first"
   * unreachable.
   */
  'set-verification-checks': [repositoryId: string, checks: VerificationCheck[]]
  /**
   * Whether a reconciler may attempt a conflicted branch here. Here as well as on the team
   * canvas, because a repository no team has claimed would otherwise have no surface at all
   * now that it is no longer an env var.
   */
