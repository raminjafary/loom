 ...
 *
 * A persona still carries a default; this overrides it for one run. The cost model names
 * model choice as *the* cost swing factor and requires it be visible rather than buried in
 * config — and editing the persona was the only way to change it, which changed it for
 * everyone and for every run after.
 */
const model = ref('')
...
/**
 * Empty means the persona's own cap. A cap only an operator could change by editing the
 * persona is a cap nobody adjusts for the one expensive run that warrants it.
 */
const budgetCap = ref('')
 ...
 *
 * A planner cannot start a worker on a higher model tier, so moving a planner down to save
 * money can leave it with nobody to delegate to — every persona correct, the whole roster
 * empty, and nothing saying so anywhere. That was measured by paying for a live run which
 * planned nothing and replied that "the only available persona is sweep-probe". The roadmap
 * calls showing the attenuation envelope at design time the composition canvas's
 * highest-value job; this is the same insight one control earlier, where the choice that
 * causes it is actually made.
 *
