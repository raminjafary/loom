...
/**
 * The kanban, which is the same object as the worker-notes ledger: building them
 * separately would produce two sources of truth for what a swarm is doing. So a card
 * here *is* a run — there is no task table
 ...
 *
 * `pathCollisions` is the part worth looking at. The worker-notes design calls path
 * ownership "the cheapest available attack on the assumption" — that the main cause of
 * merge conflict is two workers independently deciding to touch the same file — so the
 * collisions a swarm is heading for belong on the board *before* the merge queue discovers
 * them.
 */
...
/**
 * Live swarm observability: "A worker at 90% of its context is about to compact and get
 * worse." Drawn from half-full — early enough to see pressure building, late enough that an
 * idle worker's card stays quiet.
 */
