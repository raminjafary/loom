...
/**
 * The raw transcript is fetched only when asked for. It is the verbatim provider stream —
 * far larger than the thread, and the reason the event-tiering design tiers the write path
 * at all — so loading it alongside the diff would undo the point of the tiering.
 */
 ...
 * does here (repository binding: keep / discard / queue / push all follow from it), so it
 * gets the screen while it is happening and gives it back afterwards.
 *
