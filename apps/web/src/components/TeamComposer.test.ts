 ...
 *
 * Vue Flow itself is stubbed. What is worth asserting is not that a graph library renders —
 * it does — but the two rules the roadmap gives this surface: that it cannot draw an edge
 * the runtime would refuse, and that a refusal is shown in full rather than discovered one
...
describe('TeamComposer', () => {
  ...
  /**
   * Portable expertise, and the operator's case: two agents in one role, one of which
   * learned a particular subsystem. The answer is two *personas* — expertise attaches to an
