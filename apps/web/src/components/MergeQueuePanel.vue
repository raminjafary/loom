...
/**
 * The serialized merge queue (PLAN.md §7 Phase 2). Renders the order, because the
 ...
 * a way to jump the queue would be the race the queue replaces (§5a).
 */
  ...
  <section class="panel">
    ...
    <ul v-else class="list">
      <li v-for="entry in props.entries" :key="entry.id" class="row" :class="entry.status">
        ...
        <!-- Plain text, never v-html: this carries git output (PLAN.md §6 A8). -->
