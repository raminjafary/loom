 ...
 *
 * The tech stack pins Vue Flow for exactly this and not for the observability graph, and
 * the distinction is the whole design: **two canvases, not one.** The swarm graph draws
 * runs whose positions are computed from the tree's own depth ordering — facts, worth
 * nothing to persist, which is why forty lines of inline SVG beat two dependencies there.
 * This one draws personas a human arranged, where position is a choice worth keeping,
 * dragging is the interaction, and the layout is a fact about the team.
 *
...
const props = defineProps<{
   ...
   *. Keyed by persona id.
   *
   * Portable expertise asks this canvas to show, per member, what it knows — because a
   * roster of names with no expertise on it is what made "two security reviewers, one of
   * which learned this subsystem" impossible to see. Still not filtered to the team's
...
const emit = defineEmits<{
  ...
  'save-group': [
    input: {
      ...
      /** The other repositories a subtask may name. Always sent. */
...
const flowNodes = computed(() =>
  nodes.value.map((node) => ({
     ...
     * number — not N copies of a persona, which would be two runs and belongs on the *
     * board. Unsized members show nothing, so a team that never set a width looks exactly
 ...
 *
 * Deliberately destructive of the stored arrangement, and offered as a button rather than
 * done on open for exactly that reason. Position is a fact a human recorded and recomputing
 * it on every open would throw that away every time — but a team whose shape has changed
 * since is holding an arrangement that describes a team that no longer exists, and the
 * operator's report is what that looks like: a second planner added to a five-member team,
 * and a canvas with no visible hierarchy at all.
 */
const arrange = () => {
 ...
 *
 * On by default, because the parallel-branch measurement measured the cost of not having
 * it: a third of parallel branches needed roughly fifty seconds of human attention each, on
 * conflicts requiring no judgement. The safety case does not rest on the agent being right
 ...
 * member is offered to *every* planner's roster, which is what every team does today. So
 * the option says "anyone's" rather than "none" — "none" would read as a member nobody may
      ...
      <div v-if="props.groups.length > 0" class="body">
        ...
        <aside ref="side" class="side">
          ...
          <section class="lands">
            ...
            <!--
              The cross-repository fleet, drawn where the single
...
/*
 * Labels were red because until the canvas design a label only ever appeared on a *refused*
 * edge, so the colour and the meaning happened to coincide. A review edge is labelled
 * always and is not a problem, and painting its label in the danger colour made a healthy
 * expectation read as an error. The refused case keeps red; everything else takes the
 * edge's own colour.
 */
