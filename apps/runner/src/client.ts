...
export const connectRunner = (options: RunnerClientOptions): { close: () => void } => {
  ...
  // Durable per-run state (PLAN.md §7 Phase 1) — the SDK session id, the clone, and
  ...
  const runAgentForRun = async (input: {
    ...
    // explanation (PLAN.md §6's A6 caveat).
    if (!sandboxModelKey) {
      sendAgentEvent(input.runId, {
        ...
        message:
          ...
          'must be handed a real key to pass that check (see PLAN.md §6, A6 caveat). Set LOOM_SANDBOX_ENABLED=0 to run ' +
  ...
  const connect = () => {
    ...
    ws.on('open', () => {
      ...
      // before anything else happens (PLAN.md §7 Phase 1). Read from disk each
    ...
    ws.on('message', (raw: Buffer | ArrayBuffer | Buffer[]) => {
      ...
      switch (frame.type) {
        ...
        /**
         * Continue a run this Runner already has state for (PLAN.md §7 Phase 1). The
