...
export const connectRunner = (options: RunnerClientOptions): { close: () => void } => {
  ...
  /**
   * The raw transcript tier's batching: chunked JSONL, flushed on size or interval.
   *
   ...
   *
   * Redaction happens here, on the host, before the line ever reaches the socket (the
   * credential broker, "redacted at write"). Doing it server-side would mean the unredacted
  ...
  const runAgentForRun = async (input: {
    ...
    const onEvent = async (event: WireAgentEvent) => {
       ...
       * the agent. Mastery: an agent's own estimate of its progress is model output, and
       * may be a remark but never the number.
       */
    ...
    // Skills are written into the run's HOME before the SDK starts, so the registry — not
    // the clone — is where a run's skills come from. HOME is run-scoped and destroyed with
    // the run, so nothing outlives it. A Planner gets exactly one channel it can act
    // through; everything else it might want happens because the server decided to, not
     ...
     *
     * Unlike notes, this is not given to every run. A map is a persona-level artifact that
     * later runs are handed as context, so letting an ordinary worker write to it would
     * mean any run could edit what every future run reads — which is the * "one poisoned
     * run could write a lesson every future run reads", the exact risk that section says
    ...
    /** Tier 2's channel, on the same round trip and the same pending map. */
    const onToolsEdit = (edit: {
      tools: string[]
      rationale: string
    }): Promise<{ ok: true; outcome: string } | { ok: false; error: string }> => {
      const requestId = nextNoteRequestId()
      send({ type: 'persona_tools_revised', runId: input.runId, requestId, ...edit })
      return new Promise((resolve) => {
        const timer = setTimeout(() => {
          pendingSelfEdits.delete(requestId)
          resolve({ ok: false, error: 'the platform did not answer in time — nothing changed' })
    ...
    try {
      ...
      await pumpUsage().catch(() => {})
      /**
       * In the `finally` rather than beside `flushPlan`, unlike the plan: a plan from a run
       * that died should not spawn children, and the coverage of a run that died is exactly
       * what a human wants to see — mastery writes the map incrementally for the same
    ...
    ws.on('message', (raw: Buffer | ArrayBuffer | Buffer[]) => {
      ...
      switch (frame.type) {
        ...
        case 'start_run': {
          ...
          const openOnBranch = frame.openOnBranch
          // A reconciler opens onto a paused rebase in a clone of the conflicted run's
          // clone; a reviewer and a prosecutor open on the branch they are examining; every
          ...
          const prepare = reconcile
            ...
            : openOnBranch
              ? (() => {
                  const target = runWorkspaces.get(openOnBranch.targetRunId)
                  if (!target) {
                    return Promise.reject(
                      new Error(
                        `cannot open on ${openOnBranch.branchName}: this Runner no longer holds run ${openOnBranch.targetRunId}'s clone`,
                      ),
                    )
                  }
                  return prepareReviewWorkspace(
                    target.clonePath,
                    openOnBranch.branchName,
                    runId,
                    frame.defaultBranch,
                  )
                })()
        ...
        case 'verify_run': {
          ...
          if (!workspace) {
            /**
             * `error`, not `skipped`. A run whose workspace this Runner no longer holds was
             * not measured, and the two are only the same fact if you do not care which —
             * the self-improvement loop scores on these rows, and a discarded clone
             * reported as "nothing to verify" would enter the arithmetic as evidence.
             */
            send({
