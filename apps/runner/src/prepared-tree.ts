...
/**
 * The base-image half of the "a base-image and dependency-cache story is
 * required before parallel workers are useful".
 *
 * The cache half shipped: a run's `npm install` resolves offline in seconds instead
 * of minutes. What it does not remove is the install itself — a run still starts on
 * a bare clone with no `node_modules`, so every worker in a swarm pays it, and pays
 * it *as a model turn*: the agent has to notice, decide, and spend a tool call before
 * it can run one test. Repository binding says where this belongs — "it also needs the warm step,
 * which is where the base-image half of this bullet belongs" — so it is built there
 * rather than as a per-repository container image.
 *
