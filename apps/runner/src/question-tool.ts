 ...
 *
 * Mid-flight steering calls this "the smallest real piece of steering", and the design is
 * that it is **not a new mechanism**: "a clarifying question is that same gate carrying a
 * prompt and returning a string. Reuse it rather than build a second blocking channel." So
 * this round-trips exactly like `read_notes` and the approval gate — server, human, back —
 * and inherits the SLA, the auto-deny, the notification and the identity binding rather
 ...
 *
 * **The description is doing real work.** A model that asks whenever it is slightly unsure
 * turns a swarm into a queue of interruptions, which is the failure mode that makes a human
 * stop reading them — so it says when *not* to ask, and says that guessing-and-recording is
 * the cheaper default. The whole justification is reducing the human attention the riskiest
 * assumption measured as the real cost, and a chatty tool spends exactly that.
 */
...
export interface QuestionToolCallbacks {
  /**
   * Sends the question and resolves when a human answers, the gate is denied, or the SLA
   * expires. `answer: null` is "nobody answered" — never an error, because mid-flight
   * steering requires the run to continue either way: "a run blocked forever on a question
