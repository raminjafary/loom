 ...
 * modelled on `write_note` and deliberately *not* on `submit_plan`. The worker-notes design
 * states the contrast: a plan is only actionable whole, so it is collected and emitted
 * after the agent loop ends; a map is worth what it is worth the moment it exists, and the
 * runs whose output matters most here are exactly the ones that never reach a stop handler.
 * A mastery run is the longest-lived run in the system, so it is the likeliest of all of
 * them to be killed, reaped or capped mid-way.
 *
