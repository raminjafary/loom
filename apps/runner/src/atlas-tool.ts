 ...
 * here. The cap, the fence and the "leads, not facts" framing are security properties (the
 * planner/worker trust boundary: another agent's report is untrusted input forever, and a
 * report about a repository this run cannot see is the strongest case of it), and a Runner
 * that rendered its own would be a second place for them to drift.
 */
