 ...
 * imported as a library, not shelled out to as a subprocess.
 *
...
export interface RunAgentOptions {
  ...
  /**
   * The verbatim SDK message, before `toWireEvents` maps it down. Optional: an unsandboxed
   * debugging run has no reason to persist one, and a caller that does not want the tier
   * should not pay to serialize it.
   */
   ...
   *
   * The tool's *name* travels with the server rather than being derived here, because there
   * are now two of them: an ordinary Planner gets `submit_plan`, and a re-planning turn
   * gets `submit_plan_delta` instead. Deriving it would leave a second place that has to
   * agree about which one is mounted, and a disagreement there is the silent failure this
   * codebase has already shipped twice — a tool the model is never offered, with nothing to
   * notice it but a live run.
   */
 ...
 * `canUseTool` gate that is the whole point. Loom does not
...
export const runAgent = async (options: RunAgentOptions): Promise<void> => {
  const canUseTool: CanUseTool = async (toolName, input) => {
    ...
    /**
     * The whole decision, in one place and in one order (see `gateBehavior`). Both the
     * effect-based skip of effect-based classification and the persona's approval mode live
     * in there, so neither can be reordered relative to the boundary check above it.
     */
