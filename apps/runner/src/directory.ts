...
/**
 * The two Runner capabilities repository binding asks for beyond `checkPath`:
 * `listDirectory` (which "backs the web directory-picker and the TUI equivalent
 * alike") and repository creation via `git init`.
 *
 * Both are gated by the same boundary as everything else the Runner exposes: a
 * path is usable only if it resolves, *after symlinks*, inside one of this
 * Runner's configured allowed roots. Repository binding states the reason plainly — without it,
 * "an agent can be aimed at `~/.ssh`". A directory picker is a more tempting way
 * in than most, since browsing is the one operation that looks harmless.
 */
...
/**
 * Creates a new git repository under an allowed root (the "or creates
 * one (`git init`)").
 *
