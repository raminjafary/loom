 ...
 * this host, so the egress proxy can present it upstream (PLAN.md §6 A6).
 *
 ...
 *
 * Licensing, stated because it is a real constraint and not a technical one: PLAN.md §8
 * records that the SDK's terms prohibit exposing claude.ai subscription limits to *your
 * users*. A single operator running their own Loom is one reading; a team workspace on
 * one person's subscription is the case the term is about. An API key remains supported
 * for that reason.
 */
