 ...
 * Repository binding names `core.hooksPath` as exactly the way a run turns a later host-side git
 * invocation into code execution. `prepareRunWorkspace` pins both at clone time; pinning
 * them again per invocation means a clone whose config was rewritten afterwards still
 ...
 *
 * Run **in the run's own clone at its own head** — not rebased onto anything. That is
 * the whole difference from the merge queue: this asks whether the work the run
 * produced is done, before a human spends time reviewing it or a candidate is promoted
 * (continuity mode–4); the queue asks the later, separate question of whether it still
 * passes on top of everything that landed since.
 *
