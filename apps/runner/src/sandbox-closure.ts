 ...
 * out of the image because the push policy depends on the agent being unable to reach them.
 * A blanket `COPY src/ src/` would be shorter and would hand the agent the host-side half
 ...
 * worker-notes tools and the Planner's `submit_plan` were absent from every sandboxed run
 * for days, because the image predated them and rebuilding it is a manual step. Nothing
 * failed. Runs completed, work was committed, cost was metered, and the model — told to
 * call `write_note` and offered no such tool — invented a substitute by writing a markdown
 * file into the clone, which is the one place the worker-notes design says notes must never
 * go. A green suite proved nothing, because the suite exercises host-side code and the
 * container was running a different, older copy of it.
 *
