...
/**
 * Per-run container isolation. The agent's process no
 ...
 *   `--network=none` would also cut the egress proxy, and the sandbox spec's own sentence
 ...
 * risky-tool gate is a name-based heuristic, not a boundary (effect-based classification
 * says so itself).
 *
 ...
 * is running.
 *
 * The image is built by hand and is easy to forget, and forgetting is not a loud failure:
 * the container starts, the run completes, work is committed, cost is metered — and the
 * agent silently lacks whatever the newer sources added. That is not a hypothetical.
 * `notes-tool.ts`, `planner-tool.ts` and `capabilities.ts` were missing from every
 * sandboxed run for four days; a model told to call `write_note` and offered no such tool
 * wrote a markdown file into the clone instead, which is precisely where The worker-notes
 * design says notes must never go. The whole test suite was green throughout, because the
 * suite exercises the host's copy of code the container did not have.
 *
 ...
 *
 * This is a second bind mount, which the sandbox spec's "mount only the run's clone" does
 * not literally allow. The risk is the same shape as the clone's, and deliberately so: the
 * directory is created per run, owned by that run, and destroyed with it. What the sandbox
 * spec is actually guarding against — `$HOME`, `~/.ssh`, `~/.aws`, `~/.claude` on the
 * *host* — is still never mounted.
 */
const HOME_DIR = '/home/agent'


/**
 * Every flag the sandbox spec asks for, in one place so the spec is auditable against the
 * code rather than scattered through a spawn call.
 */
/**
  ...
  stdout.on('line', (line) => {
    ...
    switch (frame.t) {
      ...
      case 'variant_verdict':
        ...
        return
      case 'variants_propose':
        void (async () => {
          const result = (await options.onProposeVariants?.({
