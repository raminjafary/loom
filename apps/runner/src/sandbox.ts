...
/**
 * Per-run container isolation to PLAN.md §6 A5's spec. The agent's process no
 * longer runs with the Runner's privileges — which matters because the Runner is
 * the component that holds git credentials and push authority (§6 A2), and model
 * output is attacker-controllable input (§2 principle 11).
 *
 * Where this deviates from A5, and why — stated rather than buried:
 *
 * - **`--network` is an internal Docker network, not `none`.** A literal
 *   `--network=none` would also cut the egress proxy, and A5's own sentence is
 ...
 * - **Containers, not microVMs.** A5 is explicit that a shared kernel is not a
 *   sufficient boundary for LLM-generated code. Kata/microsandbox is Phase 3
 *   (§7); this is the Phase 1 boundary, and it is a real one compared to running
 *   unsandboxed, not a claim to have solved kernel escape.
 * - **Runtime defaults to docker, not podman.** §8 prefers podman for being
...
/**
 * Every flag A5 asks for, in one place so the spec is auditable against the code
...
export const buildSandboxArgs = (
  ...
  // adding `--security-opt seccomp=unconfined`, which A5 forbids outright.
 ...
 * secret leaving the host (§6 A6) — the token is opaque and per-run, and the real
...
export const runAgentInSandbox = async (
  ...
  // Wall-clock kill (PLAN.md §6 runtime safety). The dead-run reaper catches a
