...
describe('sandbox args meet the isolation spec', () => {
  ...
  it('never disables seccomp', () => {
    // the sandbox spec is explicit: default seccomp, never `unconfined`. Passing no seccomp
    // flag is what keeps the default, so the assertion is on the absence.
