...
export const PLAN_SUBTASKS_SCHEMA = z
  .array(
    z.object({
       ...
       *
       * It also states the cost, for the reason the collaboration topology gives: "a DAG of
       * agents is a machine for turning one bad early decision into eight expensive later
       * ones". A model that thinks sequencing is free will sequence everything, and a fully
       * serial plan throws away the 2.1× the parallel-branch measurement measured.
       */
      dependsOn: z
        .array(z.number().int())
        .optional()
 ...
 * server.
 *
 * **The descriptions are the design.** Mid-flight steering point 2 is a research finding,
 * not a preference — agents repair disrupted plans badly and make bounded local edits well
 * — so every field here pushes toward the smallest change that answers the human, and says
 * outright that no change is a valid answer. A model told only "emit a delta" will produce
 * the whole plan again with edits folded in, which is the one outcome this must not have.
 */
