 ...
 * the provider's own response rather than trusting a caller's claim.
 */
...
describe('egress proxy: credential injection', () => {
  it('swaps the sandbox lease token for the real key', async () => {
    ...
    // The whole point of the credential broker: the sandbox held an opaque token, the
    // provider saw the real credential, and the two never met.
...
/**
 * The CONNECT path's refusals, which is the half that matters for the sandbox spec. The
