...
The README is the short version. This is the long one: what each part does, why the
boundaries sit where they do, and which claims are measured rather than asserted.

The reasoning that constrains a specific piece of code lives in a comment next to that
code, deliberately — this file collects the parts that span more than one module.
...
![The Expertise panel: a booking-demo map marked on trial, six live claims at a pinned commit, and a graph whose dashed edges mark what an agent concluded rather than parsed](screenshots/expertise.png)
...
![The Colosseum panel: a convened session on booking-demo asking whether refundFor double-converts minor units, with one claim settled against a check and none left standing](screenshots/colosseum.png)
...
| **A Runner may only speak for its own runs** | Two Runners on one host is the ordinary arrangement — every live driver spawns one. A *reply* used to be matched by request id alone, so a second Runner could resolve another's in-flight merge, push, diff or verification by answering with its id; every pending request now carries the Runner it went to, taken from the connection rather than from the frame, and a reply from anyone else is dropped (rejecting would let a second Runner fail another's merge on demand). Every *unsolicited* frame — an event, a heartbeat, a plan, a note, a cost report — is checked the same way against the run it names, once above the switch, so nothing below it can be written into another Runner's run. |
...
the live drivers in `tools/` — thirty-seven of them, run by hand. Each drives a real server, a
...
`trial-traffic.mts` is the one that answers the question the whole shape rests on, and it has
answered twice.

Its **first** verdict went *against* the harness: a planner and its workers took 100% of their
tasks to the harness's 80%, on a class where the harness was cheaper and failed no checks at all.
It lost one task by producing nothing — its scoping step did not answer, so the fan never opened.
A planner that plans badly still leaves a tree of workers; a drawn shape had a single point of
failure that planner-and-delegate does not.

That hole is now closed — a step whose run finished without calling its answer tool is dealt one
more attempt — and the **second** verdict goes to the harness: outcomes level at 100% against
100%, none of the harness's five branches failing its repository's checks against two of the
planner's five, at $0.108 a task against $0.144. Both are five a side, which is a result rather
than a finding; the difference between them is not the re-run, it is the fix.
