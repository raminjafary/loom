/**
 * The gate the reconciler agent has to pass before it is wired
 ...
 *
 * **Why these scenarios.** The parallel-branch measurement measured the real population on
 * a real repository: a third of parallel branches conflicted, and every single one was
 * *additive* — two workers appending to the same list, both right. So the resolvable cases
 * here are that shape. But the number that decides whether this can run unsupervised is not
 * how often it resolves; it is **how often it refuses when it should**. A reconciler that
...
const SCENARIOS: readonly Scenario[] = [
  {
    // The parallel-branch measurement shape, reproduced exactly: this is the conflict that
    // actually happened, five times out of five, on a real repository.
    name: 'additive-list',
    file: 'Readme.md',
    content: [
      '# Project',
