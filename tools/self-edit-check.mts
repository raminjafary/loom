 ...
 * `mastery-check.mts` uses, for the same reason: a run with nothing to conclude produces a
 * prompt rewrite that is a summary of its own task, which is exactly the edit continuity
 * mode tier 1 is worst at and this driver would happily record as a pass.
 */
...
const main = async () => {
  ...
  //
  // The self-improvement loop says the archive "stops the loop re-proposing a failure it
  // already paid for", and until this check nothing did. Driven through the same use-case a
  // run's tool calls rather than by persuading a model to re-propose something, for the
  // reason section 4 gives about directives: what is under test is the mechanism, not a
  // model's taste.
  //
   ...
   * refused it — correctly, since continuity mode requires a persona to fit its own
   * envelope, or the ceiling would be a decoration on a room already taller than it. So the
   * tier-2 case
  ...
  if (search) {
    ...
    if (verdict) {
      ...
      /**
       * The blinding, end to end. The candidates' rationales are what a second model with
       * the same weights finds most persuasive, so a verdict that quotes one is a verdict
       * that was never blind — and the whole argument for a verifier rests on it being
       * blind.
       */
      check(
