/**
 * The reconciler, measured *through the merge queue*, with a
 ...
 *
 * The roadmap puts the serialized queue *underneath* the agent precisely so a wrong
 * resolution is cheap. That argument has never been observed: the one live end-to-end
 * reconcile ran against a repository with **no verification command**, so the branch the
 * agent produced was merged without a single test running. The safety net was configured
 ...
 *                            right; the union is correct and must pass verification. The
 *                            good case, and the shape the parallel-branch measurement
 *                            measured 5/5.
 ...
 * its own suite, installed offline from the repository binding cache with the network
 * closed.
 *
 *   docker compose up -d
 *   set -a && . ./.env && set +a
 ...
 *
 * This is deliberate and is the difference between a check and a coin flip: The
 * parallel-branch measurement found the conflict rate is nondeterministic (1,0,1,1,2 from
 * identical inputs), so leaving the collision to chance means most trials measure nothing
 * while still paying for the runs. What is under test here is the *queue's* behaviour once
 * a conflict exists, not whether two agents happen to collide.
 */
