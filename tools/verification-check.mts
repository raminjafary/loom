...
const main = async () => {
   ...
   * asks along the way.
   *
   * The answering is not politeness. `ask_human` is offered to **every** run the platform
   * starts — mid-flight steering is explicit that asking is not a capability, so no tool
   * list and no approval mode can withhold it — and a Haiku turn given a bare fixture
   ...
   * about the harness for something that was not the harness. What is under test here is
   * what happens *after* a run ends, so the driver clears the question and moves on.
   */
