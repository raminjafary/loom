/**
 * Live driver for the held-out screen: real server, real Runner
 * *process*, real WebSocket protocol, real git repository, real Postgres, real HTTP contract.
 *
 * docker compose up -d
 ...
 * `PersonaEditor.test.ts` drives the panel against fixture props. **Nothing had watched a screen
 * decide.** the open-items list recorded that as its own limitation, and this is the driver that closes it — with
 * the one assertion none of the three can make: that the commit a replay item pins is the commit
 * the Runner's clone actually opened at.
 *
