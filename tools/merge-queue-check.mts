/**
 * Live driver for the serialized merge queue (PLAN.md §7 Phase 2): real server,
...
const main = async () => {
  ...
  // shape a swarm actually produces (PLAN.md §11: N workers on one repo).
  ...
  // property the whole queue exists for (§5a).
  ...
  // code on the host at all — which is the §6 A5 boundary holding, not a bug.
  check(
