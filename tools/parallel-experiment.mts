/**
 * The riskiest assumption, measured.
 *
 ...
 *
 * The riskiest assumption asks for "three clones, three workers, one real repo, one
 * decomposed goal. Measure human minutes to reconcile versus doing it serially." This is
 * that, with the part a script can measure actually measured.
 *
 * **What changed since the riskiest assumption was written**: the serialized merge queue
 * exists, so reconciliation is now instrumented rather than purely manual. The queue
 * reports per branch whether it merged clean, conflicted, or failed verification. The
 * stopwatch is only needed for the branches it hands back — and that count is the headline
 * number this prints.
 *
 ...
 *
 * With no --repo it builds a small synthetic fixture, which is enough to exercise the
 * machinery but proves nothing about real code — the riskiest assumption says *one real
 * repo*, and it means it. Point this at something with genuine coupling between its parts,
 * or the conflict rate is measuring the fixture rather than the assumption.
 *
 * **This spends real tokens.** It is a hand-run driver, not a test: it asserts
 * nothing and prints what happened.
 *
 * Reading the result. The riskiest assumption already states the decision rule: "If
 * reconciliation costs more than it saves, the product is *one strong agent per task with
 * excellent intervention UX*" — still worth building, but a product in which most of Phase
 * 2 is dead weight. The numbers that decide it are wall-clock saved versus
 * branches-needing-hands, and cost is the tiebreak.
 */
...
/**
 * The decomposed goal. Deliberately three sibling tasks over the *same* area of a codebase
 * rather than three unrelated ones: the riskiest assumption is about workers on one shared
...
const main = async () => {
  ...
  /**
   * With `--reconcile`, wait for the agents the roadmap puts in front of the queue and
   * sweep again. This is the only place the whole path runs against a real model: a real
   * conflict, a real paused rebase in a real clone, a real agent editing the markers out,
   * and the mechanical queue verifying the result.
   *
