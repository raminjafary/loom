...
const main = async () => {
  ...
  // Polled *while the run is in flight*: the worker-notes design requires notes to be
  // durable before a run terminates, because a killed or reaped run never reaches a stop
  ...
  // The ledger as the platform would render it for a *sibling* — the fencing is the The
  // worker-notes design mitigation, so it has to survive the real path, not just a unit
  // test.
