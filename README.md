...
- Risky-tool classification is a hardcoded name list (`Bash`/`Write`/`Edit`/`NotebookEdit`) — PLAN.md §6 A3 already flags this as insufficient long-term; it's the documented Phase 1 starting point, not the final design.
- Sandbox hardening (network egress policy, resource limits, read-only mounts — PLAN.md §6 A5) is not implemented. The Runner executes agent runs with the same filesystem/network access as the process running it.
