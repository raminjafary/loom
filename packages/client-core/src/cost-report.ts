 ...
 *
 * Pure, and in client-core rather than in the component, for the same reason
 * `run-tree.ts` is: the contract-first rule means a TUI must be able to render the same dashboard without
 ...
 *
 * The cost model names this specifically — "Cursor's 8x swing came from worker model choice, so it
 * must be visible, not buried in config" — and it is the one grouping that answers it
 * directly, because a persona can be re-pointed at a different model but a model's
 * price is a fact.
 */
