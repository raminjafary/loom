...
/**
 * The swarm as nodes and edges: nodes are runs, edges are delegation and report.
 *
 ...
 *
 * **On not using a graph library.** The tech stack pins Vue Flow for the graph canvas, and
 * it is still the right choice for the surface the product shape calls *visual creation* —
 * dragging personas and drawing planner→worker edges to author a roster, where the human
 * owns the layout and interaction is the point. It is the wrong tool here. Vue Flow does
 * not lay out graphs; it renders a layout you supply, so a read-only view of a two-level
 * tree would need Vue Flow *and* a layout engine (dagre or elk) to draw what the depth
 * ordering below already knows. Two dependencies to render forty lines of SVG is the trade
 * the no-`v-html` rule declined for markdown, for the same reason: the honest version is
 * smaller than the integration.
 */
...
export type SwarmEdgeKind =
   ...
   *
   * The edge that joins the two surfaces live swarm observability left separate: the swarm
   * graph is run-level and a map is persona-level, so without this a human watching a swarm
...
/**
 * What verification did to one entry — derived from `verified` and `failureReason`, because
 * the canvas design asks for verification as "a node on that edge, not a boolean on the
 * entry".
 *
 ...
 *
 * **The stage is the persisted status and nothing more.** The canvas design describes the
 * machine as `queued → rebasing → verifying → merged`, and only two of those four are real:
 * the row carries `queued | merging | merged | failed | cancelled`, with rebase and
 ...
 *
 * Live swarm observability names this gap outright, and the reason it is worth closing is
 * the split-brain: "decisions are a standing record, not a note among notes". A count on a
...
export const buildSwarmGraph = (
  ...
  /**
   * Collision edges. Live swarm observability: path collisions "are an edge and should be
   * drawn as one".
   *
  ...
  for (const [order, entry] of relevant.entries()) {
    ...
    /**
     * From the run to its entry, and **the conflict rides this edge** — the canvas design:
     * the conflicted paths "belong on the edge between the entry that failed and the run
     * that owns the branch", because a conflict is a fact about the pair and the run is the
   ...
   * persona id — the same fact the fleet design records about teams. A name is the address
   * this platform already resolves everything by (`@mention`, the delegation roster, the
   * merge queue), and renaming a persona is refused outright for exactly that reason, so
   * the join is as sound here as it is there.
   *
