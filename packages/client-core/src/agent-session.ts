...
export interface AgentSnapshot {
  ...
  /**
   * Which planner may delegate to which persona, and why not. Computed server-side by the
   * rules that refuse a child start, so a canvas cannot draw a team the runtime refuses.
   *
   ...
   * Workspace spend.
   *
   * Workspace-wide rather than keyed to the watched run, unlike `swarmBoard` — and that is
   * the whole reason it is a separate fetch. The cost model asks "what is this workspace
   * costing", which no rollup of one tree can answer, and a human deciding whether a model
   * tier is worth it is asking about all of them.
   */
...
export interface AgentSession {
  ...
  /**
   * Ends a search by making one candidate the persona's prompt. The only gesture in the
   * loop that writes one.
   */
  ...
  /**
   * Switches which of several concurrent runs this client is watching. Does not stop or
   * change anything server-side — it is purely which run's approvals and diff are on
   * screen.
   */
  ...
  /**
   * Re-enters a Planner with a message and lets it change its own plan. Switches the
   * watched run to the steering turn, because that is the run a human then wants to read:
   * it is where the delta and the refusals appear.
   */
  ...
  /**
   * This repository's definition of done. The whole list, because the order is a dependency
   * order.
   */
  ...
  /**
   * Whether a reconciler may attempt a conflicted branch in this repository. The
   * operator-wide `LOOM_RECONCILER_ENABLED` still wins when it is off — this is the policy,
   * not an override of the switch.
   */
...
export const createAgentSession = (options: { api: LoomApi }): AgentSession => {
  ...
  /**
   * The watched tree's ledger and board. Both in one call for the reason the worker-notes
   * design gives for them being one object: fetched apart, a board and a note list could
   * disagree about what a swarm is doing, which is exactly the second source of truth the
   * design refuses.
   */
  ...
  const syncWatchedRun = async (agentRunId: string): Promise<void> => {
    try {
      ...
      /**
       * The merge queue rides the same pass, by that same sentence: it became part of this
       * view rather than a panel beside it when the graph started drawing it, and an entry
       * advances on the server's own sweep while a human is watching — so left on the
       * Inbox's schedule the band would be stale exactly while it is being looked at.
       *
  ...
  return {
    ...
    /**
     * Reads a draft with the server's own parser. Never patches `error`: an unparseable
     * draft is what a human is *in the middle of typing*, and routing it to the
     * session-wide error banner would put a red bar across the app on every keystroke. The
     * problems come back to the caller, which renders them beside the textarea.
     */
    ...
    async startMastery(input) {
      ...
      try {
        ...
        // Watched exactly as `startRun`'s result is: a mastery run is an ordinary run, so a
        // human must be able to see it working, steer it and stop it like any other.
