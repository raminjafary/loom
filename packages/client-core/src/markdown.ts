 ...
 * paragraph loses the structure the model used to make itself readable. The usual fix is a
 * library that emits an HTML string, and that is exactly what the no-`v-html` rule forbids:
 * no agent-authored text may reach `v-html`, because a single missed sanitizer turns a
