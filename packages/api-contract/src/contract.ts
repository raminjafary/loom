...
export const contract = {
  ...
  agentRun: {
    ...
    /**
     * Every run currently executing (PLAN.md §7 Phase 2 — a workspace may now have
     * several). Distinct from `listNeedsAttention`: that answers "what is blocked on
     * me", this answers "what is running", and with concurrency those diverge.
     */
    listActive: oc.output(z.array(AgentRunSchema)),

    /** One run's children (PLAN.md §5) — what the tree view is drawn from. */
    listChildren: oc
  ...
  /**
   * PLAN.md §3's other half — what tells a human a run needs them instead of
   * making them go and look. In the contract rather than a private endpoint of
   * apps/web for the §4c reason: a terminal client must be able to register a
