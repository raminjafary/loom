...
export const ServerEventSchema = z.discriminatedUnion('type', [
   ...
   * payload to animate, not a second source of truth about what a swarm is doing — The
   * worker-notes design refuses that, and a client that rebuilt its tree from a stream
   * would disagree with the board the moment one frame was dropped. Everything here is
...
export const RepositorySchema = z.object({
   ...
   * first failure. Run against a finished run's own branch and again against a rebased one
   * in the merge queue — the same list both times.
   */
...
export const MasteryViewSchema = z.object({
  ...
  retrievalState: z.enum(['trial', 'on', 'off']),
  /**
   * What became of the runs each claim was shown to, keyed by node id. Counts rather than a
   * score, because "outranked" is a conclusion and the human should be able to check it
...
export const SwarmBoardCardSchema = z.object({
  ...
  /**
   * Where this card is running — the runner's name and the channel the work is watched in.
   * The inbox has both; the board and the active-runs panel did not, and those are the
   * surfaces a human uses while arbitrating several agents.
   *
  ...
  /**
   * Live observability. Every field is projected from events the platform already persists,
   * in the same read as the rest of the board — live swarm observability forbids a per-tick
   * query, and these add none.
   *
   * These map onto the OpenTelemetry GenAI semantic conventions, which live swarm
   * observability asks be
 ...
 * The cost dashboard.
 *
 * The cost model asks for spend "rolled up per thread/team/workspace", metered at the
 * egress proxy, with model choice **visible** rather than buried in config — the reason
 * given is that Cursor's 8x cost swing came from worker model choice. So the groupings here
 * are not decoration: `byModel` and `byPersona` are the specific question the cost model
 * says a human must be able to answer, and both read the persona *snapshot* the run
 * carried, not the persona as it is configured today.
 *
 ...
 *
 * No `pathScope`, though continuity mode lists one. Every run writes only inside its own
 * clone, enforced in the container, and a narrower scope needs an enforcer that does not
...
/** The Phase 1 subset — read/CRUD only, no git-backed versioning yet. */
/**
...
export const PromptTrialSchema = z.object({
  ...
  arms: z.array(
    z.object({
      ...
      failed: z.number().int(),
      /**
       * Branches on this arm that failed their repository's definition of done. Carried per
       * arm rather than left inside `detail`, because a human deciding whether to keep an
       * edit reads the numbers and the sentence is for the summary.
       */
...
export const VariantSearchSchema = z.object({
   ...
   *
   * Null until it has. **It counts for nothing in the measurement** — the self-improvement
   * loop is explicit that fitness is run disposition and never a model's assessment — and
   * it is on the wire anyway because it arrives long before five decided runs an arm, which
   * is the whole period during which a human has nothing else to read.
   */
  verifier: z
    .object({
   ...
   *
   * **Null means this search has no screen** — it opened before there was one, or the
   * persona has too few decided runs to build a set from. That is not the same as "nothing
   * was admitted", and a panel that rendered them alike would be describing a refusal
   * nobody made.
   */
  screen: z
    .object({
...
/** Organizational grouping of personas, not a Team/roster. */
/**
...
export const PlanReviewSchema = z.object({
   ...
   * stored: a plan mid-flight also has waiting rows, so a stored flag would need clearing
   * at a moment nobody owns.
   */
...
export const AgentRunRelationSchema = z.enum([
  ...
  'verify',
  /**
   * The screening run — one held-out item against one arm. On the wire for the same reason
   * `verify` is, and with a sharper edge: a search opens dozens of these at once, and a
...
/**
 * Notifications. `transport: null` means this deployment has no
