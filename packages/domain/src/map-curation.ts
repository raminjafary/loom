 ...
 * 3. **Retiring is a write, not a delete** (domain expertise, the bi-temporal model).
 *    Everything here produces an `invalidatedAt` stamp and a reason. The claim stays, and
 *    "this was true until commit `abc`" stays sayable.
 *
 * **What this deliberately does not do**, and the reason is worth stating rather than
 * leaving as an omission: domain expertise wants claims *scored by outcome* — "a lesson
 * cited by three runs that merged cleanly outranks one from a run that was discarded". That
 * needs per-*claim* citation, and the platform records retrieval per map (`expertise_use`),
 ...
 * than not ranking at all. The map-level version of that measurement exists and is The
 * trial.
 */
