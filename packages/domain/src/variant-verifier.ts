 ...
 * and the last one.
 *
 * The self-improvement loop: "**A surrogate verifier**, an independent session that writes its own assertions
 * and is denied the generator's context. *Does not exist,* and it is the piece with the most
 * evidence behind it: EvoSkills reports 71.1% against 53.5% for human-curated skills using
 * exactly this construction."
 *
 * ## What it is for, given that it may not touch the fitness
 *
 * The self-improvement loop is unambiguous that "fitness is run disposition, never the model's own
 * assessment", and this is a model's assessment. So the verdict is **recorded beside the
 * measurement and never inside it**. That is not a consolation prize — it buys two things
 ...
 * homogenization and factual attrition, worst exactly where agents share a model. A verifier
 * that inherits the generator's context agrees with it."
 *
 ...
 * resolved: the self-improvement loop gives the measurement the last word on fitness and gives a human the
 * last word on the prompt, so a panel that reconciled the two would be inventing an
 * authority neither has.
 */
