 ...
 * alone: continuity mode calls it the prerequisite for all of them, and the reason is
 * structural rather than sequential. The envelope is what makes tiers 2–4 *bounded at all*,
 * so building any tier before it means building an unbounded version and retro-fitting the
 ...
 *
 * **1. An absent envelope forbids self-modification rather than permitting anything.** The
 * tempting reading of `envelope: null` is "no ceiling", and it is the wrong one: it would
 * make every persona that predates this field a self-rewriting agent with no bound, which
 * is precisely what continuity mode says must not exist. So null means *this persona may
 * not rewrite itself*, and to let it, a human first says how far. That is what turns "off
 * by default" into a real off switch rather than a permissive default — the same
 * distinction The canvas design had to draw for the reconciler's env var.
 *
 ...
 *
 * **4. Path scope is absent, and saying so is better than storing it.** Continuity mode
 * lists six fields and five are here. `attenuation.ts` already records why the sixth cannot
 * be expressed: every run writes only inside its own clone, enforced in the container, and
 * a child gets its own clone rather than a subset of its parent's. A narrower scope — a
 * persona that may touch only `packages/db/` — is a real feature and needs an enforcer
 * inside the sandbox that does not exist. Storing it now would put a control on an envelope
 * that the runtime ignores, which is the one thing the roadmap forbids of the design canvas
...
export interface EnvelopeRefusal {
   ...
   *
   * Continuity mode: "a modification that would exceed the envelope is rejected and
   * surfaced to a human as a request, not silently clamped — clamping teaches an agent to
   * probe." So every refusal carries the ask, and nothing in this file returns a narrowed
   * value.
   */
...
export const envelopeAllows = (
  ...
  /**
   * Decision 3. A planner's own `tools` are read-only by the planner/worker trust boundary,
   * so this is the list that actually says what its workers may hold — and an unchecked one
   * is a ceiling a planner can raise by editing a different field from the one being
   * guarded.
   */
