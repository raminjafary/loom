/**
 * The serialized merge queue (PLAN.md §7 Phase 2, §5a "Sibling branches converge
 * through the merge queue, not a race").
 *
 * §7 is explicit about why this exists and why it is built *first*: the target is a
...
export type MergeFailureReason =
  /** The rebase hit a conflict. The branch goes back to its owning run (§7). */
  | 'conflict'
 ...
 * about to move and produce exactly the race §5a says the queue replaces.
 *
 ...
 * §6 A5 exists to remove, and worse here than in a run, because it happens *after* a
