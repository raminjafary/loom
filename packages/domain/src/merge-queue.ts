 ...
 *
 * The roadmap is explicit about why this exists and why it is built *first*: the target is
 * a reconciler *agent*, but "a reconciler agent resolves conflicts" is a research problem,
 * so it ships behind a queue that is "always the fallback: rebase in order, run tests, and
 * on failure hand the branch back to its owning run". This module is that queue's policy —
 * deterministic, cheap, and with no model in it.
 *
...
/**
 * Verification moved out of this module and generalized. `planMergeVerification` took one
 * command and returned one boolean; a definition of done is a named, ordered list, and it
 * is the *same* list a finished run is verified against. See `verification.ts`, and
 * `runMergeEntry` for the one distinction that survived: this queue asks whether the branch
 * still passes after being rebased onto the target, which is a different question from
 * whether the run that produced it was done.
 */
 ...
 *
 * **The default is refuse.** A reviewer that read the branch and said "do not merge this"
 * is the highest-value signal the ledger carries, and every other note in the ledger only
 * ever *informs* a reader who may ignore it. If a blocker did not stop the merge, the
 * reviewing role would be advisory, which is what the collaboration topology says it must
 * not be — "its verdict has to be recordable as something other than 'completed'".
 *
 * **But a human can always merge anyway**, and the override is not a compromise — it is the
 * boundary that makes the gate acceptable at all. The blocker is *model output*: a reviewer
 * that misread the diff, or was talked into an objection by the code it was reading, would
 * otherwise hold a branch shut with no key, and a model deciding what a human may not merge
 * inverts every trust rule in the security model. So the platform refuses by default, names
 * the objections, and lets a human overrule them explicitly — never silently, and the
 * override is audited.
 *
