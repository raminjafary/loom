...
/**
 * The capability registry, and attenuation applied to it.
 *
 * Capabilities are the sharpest case of the attenuation rule. The product shape gives a
 * Planner `tools: []` on purpose; an MCP server is a route to a shell, so a child able to
 * attach one its parent lacks would make that boundary decorative — the same failure
 * `attenuateChildPersona` already prevents for tools and model tier.
 */
