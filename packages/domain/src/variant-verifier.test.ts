 ...
 *
 * The tests that matter are about what the verifier is *not* told. A blinding that leaked
 * the rationale, or that reliably put the incumbent first, would produce a verdict that
 * reads exactly like a real one — which is the failure mode the self-improvement loop names
 * when it says a verifier inheriting the generator's context agrees with it.
 */
