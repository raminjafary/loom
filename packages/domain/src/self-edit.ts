 ...
 *
 * Continuity mode calls this "the cheapest and most useful tier", and the cheapness is
 * real: the persona *is* the artifact, so a self-edit is a text replacement rather than a
 * new mechanism. What is not cheap is the part this file exists for — an agent editing the
 * document that decides what it is has a route to editing what it may *do*, and the two are
 * the same file.
 *
 ...
 * model, and continuity mode is explicit that a refusal must arrive as a request a human
 * could grant: "rejected and surfaced to a human as a request, not silently clamped —
 * clamping teaches an agent to probe."
 *
 * ## Why a prompt this persona *used* to have is a refusal too
 *
 * The self-improvement loop names the archive as one of the four pieces an evolutionary
 * loop needs, and says what it is for beyond the record: it "stops the loop re-proposing a
 * failure it already paid for". Nothing implemented that. `unchanged` below compares
 * against the *live* prompt and `proposeVariantSet`'s `duplicate` compares candidates
 * against each other, so a body byte-identical to a revision this persona already moved
 * away from was accepted — and, for a variant, opened an arm that costs five decided runs
 * to re-derive a verdict the history already holds.
 *
...
export const revisePromptBody = (input: {
  ...
  /**
   * The permission check, first, and phrased as the request continuity mode asks for.
   * Before the shape checks on purpose: a persona with no envelope should hear the same
   * sentence whether or not what it tried to write was well-formed, or the refusal doubles
   * as a probe for how long a prompt it could have written.
   */
