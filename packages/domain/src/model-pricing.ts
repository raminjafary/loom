 ...
 *
 * Prices are USD per million tokens, from a verified mid-2026 list.
...
/**
 * Capability tier, ascending. Separate from price because they are separate questions:
 * price is what a call costs, tier is how much capability a run is allowed to reach for —
 ...
 *
 * The cost model names model choice as *the* cost swing factor — "Cursor's 8x swing came
 * from worker model choice, so it must be visible, not buried in config" — and until now it
 * was buried in config: the only way to run `swe` on Opus instead of Haiku was to edit the
 * persona, which changed it for everyone and every future run.
 *
