 ...
 *
 * Continuity mode gives an agent five tiers of self-*editing* and nothing that decides
 * whether an edit helped. This is the first half of what the self-improvement loop says is
 * missing: a fitness that is not self-reported. An agent rewrites its own prompt, and the
 * platform then runs both versions and counts what happened.
 *
 ...
 *
 * **2. A tie goes to the revision.** Portable expertise sends the first run of a new
 * expertise to the *withheld* arm, because an unmeasured map should not be handed to
 * anybody. The asymmetry here runs the other way: the revision is already live — the agent
 * wrote it and the platform stored it — so putting the first run on the old prompt would
 * mean silently running a version the persona no longer has. The trial measures what is
 * happening; it does not get to quietly revert.
 *
 ...
 * judgement and the money.** The self-improvement loop asks for "a fitness that is not the
 * run's own report of itself", and a disposition satisfies that only in the sense that a
 * human supplied it: it measures what a reviewer had time to look at, and it arrives days
 * late. The verification harness runs on every finished run, needs nobody, and answers the
 * half of the question a machine is entitled to answer — *this branch is not done*. So a
 * branch that failed its checks is a **decided** run even if nobody has ruled on it, and it
 * is not a success. A branch that passed is neither: passing is the floor, and only a human
...
export const summarizePromptEffect = (
  ...
  /**
   * Cost, last. The cost model makes model choice the big swing and this is the smaller
   * cousin: a prompt every future run pays for in context is a standing charge, so the same
   * results for materially less money have earned their place and materially more have not.
   */
  if (term === 'cost') {
    return favours === 'candidate'
      ? {
