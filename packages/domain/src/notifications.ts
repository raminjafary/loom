...
/**
 * The retention hook, second half: the Inbox answers "what needs me" once a human is
 * looking, and this is what makes them look. A human is notified when a run needs them —
 * not merely able to go and check.
 *
 * `NotificationPort` (packages/application/src/ports.ts) is the boundary; the The
 * replaceability contract port table lists web push as its first adapter, with
 * email/Slack/desktop/ webhook as later swaps. Nothing in this module knows which one is in
 * use.
 */
