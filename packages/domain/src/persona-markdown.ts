 ...
 *
 * `harness.budgetCapUsd` (default null = uncapped) is enforced, not advisory: the egress
 * proxy meters real spend against it and refuses further model calls once it is passed, and
 * the Runner kills the run. Enforcement lives at the proxy because that is the only place
 * that sees what a run actually cost rather than what it reports about itself.
 *
 ...
 * and for what no mode changes (the path-scoped write boundary, the
