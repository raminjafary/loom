...
/**
 * Vague delegation is a risk, and schema-validated decomposition in both directions is
 * its mitigation. These cover both legs: what
...
describe('parseDecomposition', () => {
  ...
  /**
   * The cost model and the security model both care: a Planner that can fan out without
   * bound is how a runaway loop gets expensive, and the workspace concurrency limit only
   * bounds what runs at once, not what gets queued behind it.
   */
...
/**
 * Path ownership belongs in the decomposition. A Planner declaring which paths each
 * subtask owns lets the platform warn about overlap *before* tokens are spent, and lets
 * the merge queue predict a conflict instead of discovering it. It is the cheapest
 * available attack on the parallel-work assumption.
 */
...
/**
 * The DAG. The distinction the tests are built around is the one collaboration topology
 * draws itself: path overlap **warns** because it is a guess about the future, and a cycle
 * is **refused** because it is a statement about the plan — a plan that cannot be ordered
...
describe('describePlanStages', () => {
  ...
  it('refuses to sum around an uncapped persona', () => {
    // The cost model will not carry a second arithmetic beside the one caps are enforced
    // against, and the number's whole job is to let a human refuse a pipeline before it
    // runs — which only a worst case can do.
...
/**
 * The reviewing role — the reviewing relation, and specifically the three ways it differs
 * from `dependsOn`. Two of those differences are enforced right here (no paths, and not a
 ...
 * caught it: the warning would have fired on *every* cross-repository plan, and the whole
 * argument for it is that a human acts on it — a warning that is usually wrong is one they
 * stop reading.
 */
