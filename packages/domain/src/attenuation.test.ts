...
/**
 * PLAN.md §5's attenuation rule, one test per way a swarm could otherwise be used
 * to acquire what its parent was denied. The Planner case is the sharpest: §3 gives
...
describe('attenuateChildPersona', () => {
  ...
  it('stops a tools-[] Planner from spawning a child with tools', () => {
    // The whole point of §3's Planner trust boundary.
  ...
  it('does not impose a ranking on a self-hosted parent that has none', () => {
    // §8's open-weight path has no place in this table at all; refusing every
