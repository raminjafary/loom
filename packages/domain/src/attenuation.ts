...
/**
 * The capability attenuation: "a child run can never request tools,
 ...
 * shell (the product shape, the trust boundary) — could simply spawn a child that has them, and
 * the trust boundary would be decoration. Continuity mode later generalizes the same idea into
 ...
 *, and a child gets its own clone rather than a subset of its parent's.
 */
 ...
 */
 ...
 // runaway loop's cost (the security model/the cost model: caps are enforced, not advisory).
