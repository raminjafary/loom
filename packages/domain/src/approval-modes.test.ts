...
describe('what each mode lets through', => {
 ...
 /**
 * The line that makes `accept-edits` a defensible middle rather than a smaller
 * `auto`. A shell can write a file too — and can also push, install, and read a
 * credential — and `classifyBashCommand` triages those without being sound (effect-based classification
 * says so itself), so "accept edits" must never mean "accept a shell that happens
 * to edit".
 */
