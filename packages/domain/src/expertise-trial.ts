 ...
 * override, because domain expertise and mastery are consistent that promotion is a human act.
 * - **"No difference" is a refusal, not a pass.** An expertise that neither helps nor
 * hurts is spending context for nothing, which is exactly the failure the worker-notes design
 * bounds notes against.
 */
 ...
 *
 * `merged` and `discarded` are the outcome, and they come from the run's disposition
 * rather than from its status: a run that *completed* produced a branch, and whether that
 * branch was any good is the thing a human said afterwards by merging or discarding it.
 ...
 *
 * The override wins in both directions and is checked first, because domain expertise and mastery are
 * consistent that promotion is a human act — and the same must be true of demotion, or an
 * operator watching a map produce bad advice would have to wait for five more runs to
 * agree with them.
 */
