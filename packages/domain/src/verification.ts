 ...
 *
 * principle 6 says "every agent action needs a verifiable check — test, build, diff, or
 * second-agent review. 'Looks done' is not a stop condition." Until now the platform had
 * exactly one automated check, and it ran in the wrong place: the merge queue's
 * `verifyCommand`, executed *after* a human approved a merge. That is late (the human
 * reviewed an unverified branch, and reviewing is the expensive step), coarse (one command,
 * one boolean, so "the build broke" and "one test failed" are the same fact), and
 * unavailable to anything but a merge — while the tiers 3 and 4 need to verify a
 * *candidate* before promoting it, and the self-improvement loop needs a fitness that is
 * not the run's own report of itself.
 *
 * So the definition of done becomes a **named, ordered list of checks** belonging to the
 * repository, and it is one list: the merge queue runs it against the rebased branch, and a
 * finished run runs it against its own. Two lists would drift and nobody would know which
 * one the platform meant — the same argument the self-improvement loop makes for importing
 * the thresholds rather than restating them.
 *
 ...
 *
 * The name is not decoration. A verdict of "failed" is unactionable; "the build failed" and
 * "the tests failed" are different next actions for the human, and different signals for
 * the self-improvement loop — a candidate persona that breaks the build is worse than one
 * whose tests are flaky, and a single boolean cannot say so.
 */
 ...
 * branch — the artifact continuity mode tiers 3–4 gate promotion on and the
 * self-improvement loop scores against.
 *
 ...
 *
 * Every check is a process the platform runs per finished run and per merge, so the list is
 * a multiplier on the machine's whole workload. Eight is far above the build/test/smoke
 * continuity mode names and far below the number at which a definition of done stops being
 * one.
 */
...
/**
 * Validates an operator's list. Refuses rather than repairs, for the reason continuity mode
 * gives about self-edits: a silently dropped check is a definition of done that quietly got
...
export const parseVerificationChecks = (input: readonly VerificationCheck[]): VerificationCheck[] => {
  ...
  return input.map((check) => {
    ...
    // Names are how a result is read and how the self-improvement loop compares one run's
    // outcome to another's. Two checks called "tests" make both meaningless.
 ...
 *
 * The non-obvious clause is `refuse`, and it is unchanged from the merge queue's own: the
 * commands are the operator's, but the *code they run* is agent-authored — a test file, a
 * `package.json` script, a `Makefile` target on the branch under verification. Executing
 * that on the Runner host is arbitrary agent code with the Runner's privileges, the precise
 * exposure the sandbox spec exists to remove. So verification runs in the sandbox, and
 * without one it needs the same explicit acknowledgement an unsandboxed run needs. It is
 * never silently downgraded to host execution.
 *
