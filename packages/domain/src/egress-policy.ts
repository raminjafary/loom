 ...
 *
 * Honest scope, stated because the sandbox spec says to state it: this allowlists by
 * *host*, on the CONNECT tunnel, without decrypting it. That is deliberate — MITM'ing every
 * tunnel to inspect paths would mean a CA inside the sandbox and a much larger attack
 * surface, and it still would not close the real hole: the model API call is itself an
 * unblockable exfiltration channel, so the load-bearing control is "secrets never enter the
 ...
 *
 * the egress record's first increment, and the only one available without decrypting a
 * tunnel: the CONNECT authority is in the clear, so *who asked for what and what happened*
 * is knowable even though the traffic is not. Until now a refusal reached a Runner's stdout
 * and nowhere else, which left nobody able to answer "what did this run try to reach" after
 * the fact — an audit gap, and the reason an allowlist gets widened by guesswork.
 *
 ...
 *
 * The wording is deliberately about *what to do*: a refusal that only says "denied"
 * produces an allowlist edited by guesswork, and the egress record's whole complaint about
 * today's behaviour is that nothing records what asked. The host is quoted because it is
 * model-adjacent text and a reader should see its edges.
 */
