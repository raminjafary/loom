...
/**
 * Every built-in ships with a budget cap (PLAN.md §6/§9: caps are enforced, not
 * advisory). A seeded persona is the one most likely to be `@mention`ed before
 * anyone has thought about spend, so shipping them uncapped would make the
 * out-of-the-box path the only uncapped one. $5 matches §4e's own example.
 */
