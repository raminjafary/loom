 ...
 *
 * **A closed vocabulary, plus free text, and the order matters.** mastery names the failure a
 * mastery run produces by default: a node per file and an edge per import, "the easiest
 * possible output, and it looks like success". Free guidance alone does not fix that,
 * because a model that has been told "learn this repository" and then "focus on
 * payments" produces the same directory listing about payments. What changes the output
 * is telling it *what earns a node* for the thing being asked — and that is a paragraph
 * per focus, written once here, rather than a paragraph a human writes correctly each
 * time. The free text stays because a closed set cannot express "the parts of this that
 * bill customers", and it is the human's, not a replacement for the vocabulary.
 *
 ...
 * would be a second, weaker envelope, and the capability registry is clear that there is one.
 */
...
/**
 * What each focus asks for, in the terms mastery says a map earns its cost by: what is *not*
 * in any one file. Every one of these is written to be checkable by the reader of the
