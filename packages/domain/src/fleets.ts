 ...
 *
 * A team is a roster of roles, and a roster cannot say "three of these". The corporation buys
 * depth and the `dependsOn` buys sequencing; nothing expressed width as a design-time
 * fact, so the only thing deciding how many `swe` workers run was a Planner's judgement
 * at plan time, one goal at a time.
 *
 ...
 *
 * What a fleet is **not**, per the fleet design: N copies of a persona on the canvas. A persona is
 * a template and a canvas node is a *role*; two nodes of one persona would be two *runs*,
 * which is a runtime fact belonging on the board. A fleet is one node carrying a number.
 */
 ...
 *
 * The fleet design is explicit — "bounded above by the workspace limit, never widening it, which
 * would make a design-time field a way around an operator's ceiling". Null when the
