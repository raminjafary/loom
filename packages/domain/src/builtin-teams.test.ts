...
describe('BUILTIN_TEAMS', () => {
  ...
  /**
   * The corporation: the chain of command starts with a decomposition, so a root that is
   * not a planner would make every tier the canvas draws below it a drawing of a tree no
   * run can have — which is exactly the refusal `updatePersonaGroup` makes for a human.
   */
...
/**
 * The two fields the operator asks added, asserted here for the reason every other rule in
 * this file is: seeding writes through the port, so a shipped team never meets the
 * validation a human's edit does. A bad one has to fail a build.
 */
