 ...
 * failing says nothing about a modification, and treating it as a regression would make every
 * drill un-runnable on a repository with one known-broken check. This is the rule about
 * `skipped` and `refused`, one level up.
...
export interface ManifestCheck {
 /**
 * What ran. Names rather than indices, because the whole point of the named checks is that
 * "failed" is unactionable and "the boundary guard failed" is a next action.
 */
 ...
 *
 * Load-bearing rather than provenance: it is what the recovering process is pinned at, which is
 * the only reason "without the modified code participating" is a structural property and not a
 * promise. A manifest with no commit is the same rumour mastery refuses in a map.
 */
