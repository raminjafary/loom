...
describe('describeDelegationRoster', () => {
  ...
  it('offers the built-in planner every built-in worker', () => {
    ...
    const spec = planner({
       ...
       * and was not: the spec defaulted to `ask` while the seed says `auto`. The autonomy
       * exposed it — with the workers at `auto` and the spec at `ask`, the data model
       * refuses every one of them, which is the shipped-roster failure this test exists to
       * catch.
       */
 ...
 *
 * Worth its own block because of how it was found: the reconciler fell off every roster
 * only because it was the one built-in at `auto` under planners at `ask`, so the
 * attenuation refused it. Making the roster autonomous removed the side effect and revealed
 * there was no rule underneath — an accidental regression, caught by a test that was
