...
/**
 * The planner/worker trust boundary, as amended twice. These tests state the boundary rather than the list:
 ...
 * the planner/worker trust boundary's own line ("reading is not acting") already said. See `planner-tools.ts` for why
 * these were excluded anyway, and what the operator changed.
 */
 ...
 * text to become planner *execution*, which is the threat the planner/worker trust boundary actually names.
 */
