 ...
 * boundary — effect-based classification says so, and the sandbox is what actually bounds a
 * run. What has changed since Phase 1 is that `Bash` is no longer gated *only* by
 * this name: `classifyToolEffect` below now classifies the command's effects, so
 * this set answers "could this tool ever matter", and the classifier answers
 * "does this call".
 */
...
/**
 * The effect-based gating, in the two forms reachable without a
 * sandbox rewrite.
 *
 ...
 * gate. This is the approval-fatigue half of effect-based classification's complaint, and it is the only
 * place this file reduces gating.
