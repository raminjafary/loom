...
/**
 * Agent-authored notes are data, not instructions, and the distinction is load-bearing:
 * a note written by worker A is read by worker B, so
...
describe('renderNotesForPrompt', () => {
  ...
  /**
   * The worker-notes design: "the **platform** separately writes the structural facts it
   * knows first-hand — branch, status, files changed, cost — which the UI must show as
   * distinct from agent prose." The model's context needs the same separation: a worker
   * that cannot tell an observed fact from a claimed one can trust neither.
   */
