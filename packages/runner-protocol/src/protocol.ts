...
/**
 * One step of a repository's definition of done. Named, because a verdict of "failed" is
 * unactionable and "the build check failed" is not.
 */
...
export const PlanSubtaskSchema = z.object({
  ...
  /**
   * Which repository this subtask lands in, by **name**. Absent or null means the planner's
   * own, which is every subtask a single-repository team writes.
   *
   * Declared here rather than left to ride the payload, for the reason this file's
   * `egressHosts` comment gives and this repository has now paid for five times: a Zod
   * schema **strips what it does not name**. A subtask's repository dropped at the frame is
   * a plan that silently lands every branch in the planner's own repository, with nothing
   * anywhere saying so.
   */
...
export const RunnerFrameSchema = z.discriminatedUnion('type', [
  /**
   * `resumableRunIds` are runs this Runner still holds on-disk state for. Sent on every
   * connect, including the first, where it is empty.
   *
   ...
   */
  /**
   * A Planner's decomposition, relayed after it submitted one. The server validates it
   * again with the domain schema before acting: the Runner is trusted to relay, not to
   * decide what a valid plan is, and the subtasks become *runs* — the most expensive thing
   * a bad payload could cause.
   */
  z.object({
   ...
   * being a generator with a second vote. The server maps the letter back by recomputing
   * the same deterministic blinding.
   */
  z.object({
  ...
  /**
   * Periodic liveness signal while a run is in flight — deliberately a sibling of
   * `agent_event`, not folded into `AgentEventSchema`: it must never become a chat message,
   * only bump `agent_run.last_heartbeat_at`.
   */
  z.object({
     ...
     *
     * Read from the SDK's `getContextUsage()`, which is a **platform fact**: it counts the
     * system prompt, tools, MCP surface and messages against the model's real window. The
     * event-tiering design models no token usage on `AgentEvent` and says token deltas are
     * stream-only, so this is the only honest source — and it is better than a sum of
     * deltas anyway, because compaction resets occupancy while a running total only ever
     * climbs.
     *
   ...
   * `total_cost_usd`: the credential broker's point is that the number a run reports about
   * itself is not the number to bill or to enforce a cap against.
   *
   ...
   * giving it one would be a third authenticated surface for a process whose whole job is
   * to be the boundary.
   *
   ...
   *
   * `host` is untrusted text: it came off a CONNECT line a sandboxed process wrote. Length
   * is bounded at what a DNS name can be, and everything downstream treats it as data.
   */
  z.object({
...
export const ServerFrameSchema = z.discriminatedUnion('type', [
  ...
  z.object({
     ...
     *
     * A held-out item is `(repository @ commit, task, observed outcome)`, so a screening
     * run has to face the tree the original run faced; replaying at whatever HEAD has
     * become means two candidates screened a day apart were screened on different problems,
     * and a control that drifts is not a control.
     *
     * A workspace preparation detail rather than a run *kind*, which is why it sits here
     * and not inside a `screen: {...}` object beside `mastery` and `reconcile`. **A sha the
     * clone
    ...
    /**
     * What this persona already knows about the subject it is working on, already selected,
     * rendered and fenced by the server.
     *
  ...
  /**
   * Merge one queued branch into its repository's default branch. Sent by the server's
   * queue sweep, one at a time per repository — the serialization is the server's, so the
   * Runner does exactly what it is told and holds no queue of its own.
   *
   ...
   * `atlas_link_result` does and one more that is specific to this tier: continuity mode
   * requires a refused self-modification to reach the agent as a request a human could
   * grant, so the wording *is* the feature and a Runner writing its own would be a second
   * place for it to drift. `error` stays for the case where the platform could not decide
   * at all.
   */
  z.object({
