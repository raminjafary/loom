 ...
 *
 * In its own file rather than appended to `agent-repositories.ts` because its rows outlive the
 * search they were written for: a rejected candidate and the reason it was rejected are the
 * buffer the piece 3 hands to a proposer, and that is a different lifetime from "which
 * arm is this run on".
 */
 ...
 *
 * Restated rather than imported because that one is a module-private `sql` fragment bound to
 * its own join shape. If a fourth definition of "decided" ever appears, this is the second and
 * they must be reconciled rather than allowed to drift — which is exactly what the roadmap says about
...
.limit(limit)
 ...
 /**
 * Context, never the score — see `ReplayOutcome`. Three values, and the one collapse
 * worth naming: a run decided only by a *failing definition of done*, with no human
 * involved, lands in `discarded`. For a reader deciding what kind of tasks a set
 * is made of, "the branch was not taken" is the fact, and `failed` is reserved for a
