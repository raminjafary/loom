...
export interface RepositoryRepositoryPort {
  ...
  /**
   * Whether a reconciler may attempt a conflicted branch here — the per-repository half of
   * what `LOOM_RECONCILER_ENABLED` used to decide for a whole process.
   */
...
export interface PlanSubtaskRepositoryPort {
  ...
  /**
   * Finds the plan a started child belongs to, so a child reaching a terminal state can
   * release or skip whatever was waiting on it. Null for a child that was not started from
   * a recorded plan — every run that predates the collaboration topology, and every run
   * started by a human.
   */
 ...
 *
 * `personaName` and `model` are read out of the run's **persona snapshot**, not out of the
 * live persona: the cost model exists because "Cursor's 8x swing came from worker model
 * choice", and a rollup that read today's persona would attribute last week's Opus spend to
...
export interface AgentRunRepositoryPort {
  ...
  /**
   * Runs a human hasn't finished with yet: awaiting an approval decision, or terminal with
   * an unreviewed branch (kept/discarded not yet decided). A reaper-failed run naturally
 ...
 *
 * Separate from `PersonaVariantRepositoryPort` because it answers a different question.
 * That port is about a search — what is being measured, and by which runs. This one is
 * about whether a candidate should have been measured at all, and its rows outlive the
 * search: a rejected candidate and the reason it was rejected are the buffer the piece 3
 * hands to a proposer.
 */
...
export interface PersonaGroupRepositoryPort {
  ...
  update(
    ...
    id: PersonaGroupId,
    /**
     * `layout` absent leaves the stored positions alone, and `fleet` absent leaves the
     * stored widths alone — a client that draws neither means "do not touch
     * them", not "clear them".
     */
    patch: {
      name: string
...
export interface AgentRunEventRepositoryPort {
  ...
  /**
   * What each of these runs is doing right now, as a projection of events already
   * persisted.
   *
...
export interface RunDispatchPort {
  ...
  startRun(input: {
    ...
    task?: string
    /**
     * Open the clone at this commit rather than at HEAD. A sha the clone does not contain
     * fails the run, deliberately: a screening run that quietly opened at HEAD would report
     * a verdict about a different problem.
     */
    baseCommitSha?: string
    /**
     * The tree's worker-notes ledger, already rendered and already fenced — absent for the
     * first run in a tree, which has no shared context to be given.
     *
    ...
    /**
     * What this persona already knows about the subject it is working on, selected,
     * rendered and fenced server-side for the same reason `contextLedger` is.
     */
    mapContext?: string
    /**
   ...
   *
   * `answer: null` is a refusal — denied, or auto-denied by the SLA. Mid-flight steering:
   * "a run blocked forever on a question nobody saw is worse than a run that guessed and
...
export interface SubjectMapRepositoryPort {
   ...
   *
   * Domain expertise asks for claims "ranked by the dispositions of runs that cited them",
   * and this is that join: per node, how many of the runs it was rendered into merged, were
   * discarded, or failed. Undecided runs are excluded for the reason
   * `tallyExpertiseOutcomes` gives — a run still in flight is not evidence either way.
   *
   ...
   *
   * "Decided" includes a branch that failed its repository's definition of done, which is
   * the same definition `tallyTrialOutcomes` uses. Keeping them identical is deliberate:
   * the map trial and the prompt trial share their thresholds, so their arm counts have to
   * mean the same thing.
   */
